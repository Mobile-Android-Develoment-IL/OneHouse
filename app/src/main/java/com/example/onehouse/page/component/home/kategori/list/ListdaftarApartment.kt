...
fun ListdaftarApartment(
    ...
    Card(
        ...
        modifier = modifier
            ...
            .padding(3.dp)
            .clickable {
                navController.navigate("detailApart/${apart.id}")
            }
    ) {
        Column(
            ...
            Text(text = apart.titleaprt)
            Spacer(modifier = Modifier.height(2.dp))
            Row {
                Image(painter = painterResource(id = apart.iconlok), contentDescription = "map", modifier = Modifier.size(15.dp))
                Spacer(modifier = Modifier.width(10.dp))
                Text(text = apart.alamatapart)
            }
            Text(text = apart.hrgaprt)
        }
    }
}