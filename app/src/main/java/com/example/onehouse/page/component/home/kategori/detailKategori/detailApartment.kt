...
fun detailApartment(
    ...
    val listApart = data.dtApartment.filter { apart ->
        apart.id == ApartId
    }
    Column(
        modifier = Modifier
    ) {
        detailApartContent(navController = navController, listApart = listApart)
    }

}

@Composable
fun detailApartContent(navController: NavController,
                       modifier: Modifier = Modifier,
                       listApart: List<MdaftarApartment>,) {
    Column (modifier = Modifier.padding(top = 40.dp),
        ...
        Column {
            ...
            Row {
                Image(
                    painter = painterResource(id = listApart[0].iconlok),
                    contentDescription = "map",
                    modifier = Modifier.size(20.dp)
                )
                Spacer(modifier = Modifier.width(10.dp))
                Text(text = listApart[0].alamatapart, fontSize = 20.sp)
            }
            Spacer(modifier = Modifier.height(10.dp))
